 ...
 *
 */
const idUrl     = 'module-get-ssl';
const idForm    = 'module-get-ssl-form';
const className = 'ModuleGetSsl';
...
const ModuleGetSsl = {
	...
	getSsl(){
		$('#div-waiting').show();
		$('#div-result').hide();
		$.get( `/pbxcore/api/modules/${className}/get-cert`, function( result ) {
	...
	checkResult(){
		$.get( `/pbxcore/api/modules/${className}/check-result`, function( result ) {
		...
		switch (status) {
			...
			case 'Updating':
				window[className].$moduleStatus
					...
					.removeClass('red')
					.addClass('grey');
				window[className].$moduleStatus.html(`<i class="spinner loading icon"></i>${globalTranslate.module_get_sslUpdateStatus}`);
				break;
			default:
				window[className].$moduleStatus
					.removeClass('green')
